...
open class Cache<T: NSCoding> {
    ...
    open func setObjectForKey(_ key: String, cacheBlock: (CacheBlockClosure, ErrorClosure) -> Void, completion: @escaping (T?, Bool, NSError?) -> Void) {
    ...
    open func objectForKey(_ key: String, returnExpiredObjectIfPresent: Bool = false) -> T? {
    ...
    fileprivate func read(_ key: String) -> CacheObject? {
        ...
        if let path = self.urlForKey(key).path , self.fileManager.fileExists(atPath: path) {
